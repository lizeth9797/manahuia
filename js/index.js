...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Inicializa el carrusel después de agregar las tarjetas
    const myCarousel = new bootstrap.Carousel(document.getElementById('carouselExampleControls'), {
        interval: 5000,
        wrap: false
    });

    // Agrega un evento de clic al contenedor de las tarjetas para manejar clics en los botones del carrusel
    document.getElementById('carouselExampleControls').addEventListener('click', function (event) {
        const target = event.target;
        const isPrevButton = target.getAttribute('data-slide') === 'prev';
        const isNextButton = target.getAttribute('data-slide') === 'next';

        if (isPrevButton || isNextButton) {
            myCarousel.isSliding = false;
            myCarousel[isPrevButton ? 'prev' : 'next']();
        }
    });
